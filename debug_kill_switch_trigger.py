#!/usr/bin/env python3
"""
Debug script to find the source of kill switch trigger
"""
...
async def debug_kill_switch_trigger():
    """Debug kill switch trigger source"""
    
    print("🔍 Debugging Kill Switch Trigger")
    print("=" * 50)
    
    # Load preset
    preset = load_preset("breakout_v1_ultra_relaxed")
    print(f"📋 Using preset: {preset.name}")
    print(f"   Kill switch limit: {preset.risk.kill_switch_loss_limit:.1%}")
    print(f"   Daily risk limit: {preset.risk.daily_risk_limit:.1%}")
    print()
    ...
    if not risk_result['approved']:
        ...
        if 'metrics' in risk_status:
            ...
            print(f"     Daily risk used: {metrics.daily_risk_used:.2%}")
            print(f"     Open positions: {metrics.open_positions_count}")
            
            # Check kill switch conditions manually
            print(f"   Kill switch analysis:")
            print(f"     Max drawdown vs limit: {metrics.max_drawdown:.2%} vs {preset.risk.kill_switch_loss_limit:.2%}")
            print(f"     Drawdown check: {metrics.max_drawdown > preset.risk.kill_switch_loss_limit}")
            
            extreme_daily_loss = preset.risk.daily_risk_limit * 3
            daily_loss_pct = abs(metrics.daily_pnl) / metrics.total_equity
            print(f"     Daily loss vs extreme: {daily_loss_pct:.2%} vs {extreme_daily_loss:.2%}")
            print(f"     Daily loss check: {daily_loss_pct > extreme_daily_loss}")
...
if __name__ == "__main__":
    asyncio.run(debug_kill_switch_trigger())
