#!/usr/bin/env python3
"""
Debug script to analyze why signals get stuck in SIZING state
"""
...
async def debug_sizing_issue():
    """Debug why signals get stuck in SIZING state"""
    
    print("🔍 Debugging SIZING State Issue")
    print("=" * 50)
    ...
    if not risk_result['approved']:
        ...
        print(f"   Risk status: {risk_result.get('risk_status', {})}")
        return
    ...
    if position_size:
        print(f"   Position size: {position_size.quantity:.2f} {signal.symbol.split('/')[0]}")
        print(f"   Notional USD: ${position_size.notional_usd:,.2f}")
        print(f"   Risk USD: ${position_size.risk_usd:,.2f}")
        print(f"   Risk R: {position_size.risk_r:.2f}")
        print(f"   Is valid: {position_size.is_valid}")
        if not position_size.is_valid:
            print(f"   Invalid reason: {position_size.reason}")
            return
    else:
        print("   ❌ No position size calculated")
        return
    
    print()
    print("🎉 SIZING TEST PASSED!")
    print("   Signal should proceed to EXECUTION state")
    print("   The issue might be in the engine's state transition logic")

if __name__ == "__main__":
    asyncio.run(debug_sizing_issue())
