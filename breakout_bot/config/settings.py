...
def validate_preset(preset: TradingPreset) -> bool:
    ...
    try:
        ...
        # Check TP fields if both are present
        if preset.position_config.tp1_r is not None and preset.position_config.tp2_r is not None:
            if preset.position_config.tp1_r >= preset.position_config.tp2_r:
                raise ValueError("TP2 must be greater than TP1")
        
        if preset.position_config.tp1_size_pct is not None and preset.position_config.tp2_size_pct is not None:
            total_tp_size = (preset.position_config.tp1_size_pct + 
                            preset.position_config.tp2_size_pct)
            if total_tp_size > 100.0:  # Size is in percentage (0-100), not 0-1
