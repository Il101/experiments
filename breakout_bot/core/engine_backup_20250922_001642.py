...
class OptimizedOrchestraEngine:
    ...
    async def _handle_initializing_state(self) -> None:
        ...
        try:
            ...
            context = LogContext(component="engine", state="error")
            self.enhanced_logger.error(f"Initialization failed: {e}", context)
            self.current_state = TradingState.ERROR
    ...
    async def _handle_scanning_state(self) -> None:
        ...
        try:
            ...
            self.enhanced_logger.error(f"Scanning failed: {e}", context)
            self.current_state = TradingState.ERROR
            self.last_error = f"Scanning failed: {e}"
    ...
    async def _transition_to_state(self, new_state: TradingState, reason: str = ""):
        """Transition to a new state and notify WebSocket clients."""
        if self.current_state != new_state:
            ...
            logger.info(f"State transition: {self.previous_state.value if self.previous_state else 'None'} -> {new_state.value} ({reason})")
